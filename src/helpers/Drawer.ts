...
class Drawer {
     ...
     */
    constructor(buffer: AudioBuffer, parent: HTMLElement, onSeek?: (time: number) => void, updateProgress?: (progress: number) => void) {
        this.buffer = buffer;
        this.parent = parent;
        ...
        this.updateProgress = updateProgress || ((progress) => console.log("Progress updated:", progress)); // Якщо немає функції, логуватимемо прогрес
    }
    ...
    // Додаємо можливість перетягування курсору (drag)
    ...
    public generateWaveform(audioData: number[], options: IOptions = {}) {
        ...
        const yScale = d3
            .scaleLinear()
            .domain(domain.map(i => Number(i))) // або просто domain, якщо дані вже числові
        ...
        // Прозорий rect для повного охоплення області (можна використовувати для кліків, якщо потрібно)\n
        this.svg
            .append('rect')
            .attr('width', width)
            .attr('height', height)
        ...
        // Малюємо часову шкалу (якщо потрібно)\n
    ...
    public clearData() {
        const rawData = this.buffer.getChannelData(0); // працюємо з одним каналом
